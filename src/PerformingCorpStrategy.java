...
public class PerformingCorpStrategy implements IComputationStrategy {
	...
	public double method(PortfolioItemAdapter item) {
		...
		double pd_value_mev1yfaiz = item.getItem().getC_mev1yfaiz() * item.getItem().getMev1yfaiz();
		double pd_value = item.getItem().getIntercept() + pd_value_m3 + pd_value_gsyh + pd_value_mev1yfaiz;
